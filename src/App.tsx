...
const chipStyles: Record<string, string> = {
  作曲: "bg-rose-50 text-rose-700 border-rose-200",
  編曲: "bg-violet-50 text-violet-700 border-violet-200",
  調声: "bg-sky-50 text-sky-700 border-sky-200",
  修正: "bg-amber-50 text-amber-700 border-amber-200",
  ミックス: "bg-emerald-50 text-emerald-700 border-emerald-200",
  マスタリング: "bg-fuchsia-50 text-fuchsia-700 border-fuchsia-200",
  その他: "bg-slate-50 text-slate-700 border-slate-200",
};

const statusMeta: Record<TaskStatus, { label: string; sectionClass: string; badgeClass: string }> = {
  active: {
    label: "進行中",
    sectionClass: "border-violet-100 bg-violet-50/50",
    badgeClass: "border-violet-200 bg-violet-100 text-violet-700",
  },
  frozen: {
    label: "凍結",
    sectionClass: "border-sky-100 bg-sky-50/50",
    badgeClass: "border-sky-200 bg-sky-100 text-sky-700",
  },
  completed: {
    label: "完了",
    sectionClass: "border-emerald-100 bg-emerald-50/50",
    badgeClass: "border-emerald-200 bg-emerald-100 text-emerald-700",
  },
  archived: {
    label: "アーカイブ",
    sectionClass: "border-slate-100 bg-slate-50/50",
...
function StatCard({ label, value, accent }: { label: string; value: string; accent: string }) {
  return (
    <Card className="rounded-[24px] border bg-white/85 shadow-sm backdrop-blur">
...
export default function DtmWorklogPrototype() {
  ...
  async function startTimer() {
    ...
    let startAt = new Date(now.getTime() + (offsets[startOffset] ?? 0) * 60_000).toISOString();
    if (startOffset === "manual") startAt = new Date(now.getTime() - 15 * 60_000).toISOString();
    const stamp = nowIso();
  ...
  async function stopTimer(taskId: Id, note?: string) {
    ...
    }), `${tasksById[taskId]?.title ?? "タスク"} を終了しました`);
    if (!ok) return;
    setEndNote("");
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(to_bottom,_#fffdfd,_#fafbff)] p-6 text-slate-900">
      ...
      <div className="mx-auto max-w-7xl">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="text-3xl font-bold tracking-tight text-slate-900">実運用寄りにしたプロトタイプ</h1>
            <p className="mt-2 text-sm text-slate-600">IndexedDB 保存、バックアップ、編集機能、保存前の最新状態確認を入れた版です。</p>
          </div>
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <StatCard label="今日の記録時間" value={formatMinutes(todayRecordedMinutes)} accent="text-violet-700" />
            <StatCard label="進行中" value={`${runningTimers.length}件`} accent="text-sky-700" />
            <StatCard label="進行中タスク" value={`${groupedTasks.active.length}件`} accent="text-pink-700" />
            <StatCard label="最終バックアップ" value={appMeta.lastBackupAt ? formatDateTime(appMeta.lastBackupAt) : "未実施"} accent="text-emerald-700" />
          </div>
        </motion.div>

        {!appMeta.onboardingSeenAt && (
          <Card className="mb-4 rounded-[24px] border border-sky-200 bg-sky-50 shadow-sm">
            <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="font-medium text-sky-900">初回案内</div>
                <div className="text-sm text-sky-700">このアプリのデータはこのブラウザ内に保存されます。別のブラウザや別PCでは引き継がれないので、定期的にバックアップを書き出してください。</div>
              </div>
              <Button className="rounded-full bg-sky-600 text-white hover:bg-sky-700" onClick={dismissOnboarding}>了解</Button>
            </CardContent>
          </Card>
        )}

        {shouldShowBackupReminder && (
          <Card className="mb-4 rounded-[24px] border border-violet-200 bg-violet-50 shadow-sm">
            <CardContent className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between">
              <div>
                <div className="font-medium text-violet-900">バックアップ推奨</div>
                <div className="text-sm text-violet-700">最後のバックアップから2週間以上経っています。JSONを書き出しておくと安心です。</div>
              </div>
              <div className="flex gap-2">
                <Button className="rounded-full bg-violet-600 text-white hover:bg-violet-700" onClick={exportBackup}><Download className="mr-2 h-4 w-4" />今すぐバックアップ</Button>
                <Button variant="outline" className="rounded-full border-violet-200 text-violet-700" onClick={dismissBackupReminder}>あとで</Button>
              </div>
            </CardContent>
          </Card>
        )}
        ...
        {notice && (
          <Card className="mb-4 rounded-[20px] border border-emerald-200 bg-emerald-50 shadow-sm">
            <CardContent className="p-3 text-sm text-emerald-800">{notice}</CardContent>
          </Card>
        )}

        <div className="grid gap-6 lg:grid-cols-[270px_minmax(0,1fr)]">
          <Card className="h-fit rounded-[28px] border border-violet-100 bg-white/90 text-slate-900 shadow-sm">
            <CardContent className="p-4">
              <div className="mb-4 flex items-center gap-2 text-sm text-slate-500"><Sparkles className="h-4 w-4 text-violet-400" /> 画面切り替え</div>
              ...
              <div className="mt-6 rounded-[24px] border border-violet-100 bg-violet-50/60 p-4 text-sm text-slate-600">
                <div className="mb-2 font-medium text-slate-900">この版で入れたこと</div>
          ...
          <motion.div key={tab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            {tab === "home" && (
              <>
                <Card className="overflow-hidden rounded-[30px] border border-pink-100 bg-white/85 shadow-sm">
                  <div className="h-2 bg-gradient-to-r from-rose-200 via-violet-200 to-sky-200" />
                  <CardContent className="flex flex-col gap-6 p-6">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div>
                        <div className="text-2xl font-bold">ホーム</div>
                        <div className="mt-1 text-sm text-slate-500">日常の開始・終了・確認をここで完結させる想定です。</div>
                      </div>
                      <div className="flex gap-3">
                        <Button className="h-12 rounded-full bg-violet-600 px-6 text-white shadow-sm hover:bg-violet-700" onClick={() => setTab("start")}><Play className="mr-2 h-4 w-4" />作業を始める</Button>
                        <Button variant="outline" className="h-12 rounded-full border-violet-200 bg-white px-6 text-violet-700 hover:bg-violet-50" onClick={() => setTab("end")}><Square className="mr-2 h-4 w-4" />作業を終える</Button>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
                  <Card className="rounded-[30px] border border-violet-100 bg-white/85 shadow-sm">
                    <CardHeader>
                      <SectionTitle icon={<Clock3 className="h-5 w-5" />} title="進行中の作業" sub="並列進行しているものが一目で分かるようにする" />
                    </CardHeader>
                          ...
                          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <Badge className={`rounded-full border ${chipStyles[item.workType?.name ?? "その他"] ?? chipStyles["その他"]}`}>{item.workType?.name}</Badge>
                              </div>
                            ...
                            <Button className="rounded-full bg-white text-violet-700 shadow-sm ring-1 ring-violet-100 hover:bg-violet-50" onClick={() => stopTimer(item.taskId)}>この作業を終える</Button>
                          </div>
                        </div>
                      ))}
                    </CardContent>
                  ...
                  <div className="grid gap-6">
                    <Card className="rounded-[30px] border border-amber-100 bg-white/85 shadow-sm">
                      <CardHeader>
                        <SectionTitle icon={<FolderOpen className="h-5 w-5" />} title="すぐ再開しやすい進行中タスク" sub="凍結はここには出さない" />
                      </CardHeader>
                      <CardContent className="space-y-3">
                        {activeTasks.length === 0 ? (
                          ...
                          <div key={task.id} className={`flex items-center justify-between rounded-[22px] border p-3 ${index % 2 === 0 ? "border-amber-100 bg-amber-50/80" : "border-violet-100 bg-violet-50/80"}`}>
                            <div>
                    ...
                    <Card className="rounded-[30px] border border-emerald-100 bg-white/85 shadow-sm">
                      <CardHeader>
                        <SectionTitle icon={<FileText className="h-5 w-5" />} title="直近ログ" sub="時計ズレや記録ミスを後から直せる" />
                      </CardHeader>
                      <CardContent className="space-y-3">
                        {recentLogs.length === 0 ? (
                          ...
                          <div key={log.id} className={`rounded-[22px] border p-3 ${index % 2 === 0 ? "border-emerald-100 bg-emerald-50/80" : "border-pink-100 bg-pink-50/80"}`}>
            ...
            {tab === "start" && (
              <div className="grid gap-6 xl:grid-cols-[1.08fr_0.92fr]">
                <Card className="rounded-[30px] border border-pink-100 bg-white/85 shadow-sm">
                  <CardHeader>
                  ...
                  <CardContent className="space-y-6">
                    <div>
                      ...
                      <div className="mt-3 grid gap-3 sm:grid-cols-2">
                        {filteredStartTasks.length === 0 ? (
                          <div className="col-span-2 rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">進行中タスクがありません。まずはタスク一覧で追加してください。</div>
                        ) : filteredStartTasks.map((task, index) => {
                          const active = selectedTaskId === task.id;
                          return (
                            <button key={task.id} onClick={() => setSelectedTaskId(task.id)} className={`rounded-[24px] border p-4 text-left transition ${active ? "border-violet-300 bg-violet-100 text-violet-900 shadow-sm ring-2 ring-violet-200" : index % 2 === 0 ? "border-pink-100 bg-pink-50/70 hover:border-pink-200" : "border-sky-100 bg-sky-50/70 hover:border-sky-200"}`}>
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <div className="font-semibold">{task.title}</div>
                                  <div className="mt-1 text-sm text-slate-500">{task.category} ・ {task.memo ?? "メモなし"}</div>
                                </div>
                                {active ? <CheckCircle2 className="h-5 w-5 shrink-0 text-violet-700" /> : null}
                              </div>
                            </button>
                          );
                        })}
                      </div>
                      <div className="mt-3 rounded-[24px] border border-dashed border-violet-200 bg-violet-50/50 p-4 text-sm text-violet-700">新しいタスクは「タスク一覧」画面で追加する想定です。</div>
                    </div>

                    <div>
                      <div className="mb-2 flex items-center justify-between gap-3">
                        <label className="block text-sm font-medium">2. 作業項目を選ぶ</label>
                        <Badge className="rounded-full border border-violet-200 bg-violet-50 px-3 py-1 text-violet-700">選択中: {workTypesById[selectedWorkTypeId]?.name ?? "未選択"}</Badge>
                      </div>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {activeWorkTypes.map((option) => {
                          const active = selectedWorkTypeId === option.id;
                          return (
                            <button key={option.id} onClick={() => setSelectedWorkTypeId(option.id)} className={`rounded-[24px] border p-4 text-left transition ${active ? "border-violet-300 bg-violet-100 text-violet-900 shadow-sm ring-2 ring-violet-200" : `${chipStyles[option.name] ?? chipStyles["その他"]} hover:shadow-sm`}`}>
                              <div className="flex items-start justify-between gap-3">
                                <div>
                                  <div className="font-semibold">{option.name}</div>
                                  <div className={`mt-1 text-sm ${active ? "text-violet-700" : "text-slate-500"}`}>記録したい作業項目をここで選ぶ</div>
                                </div>
                                {active ? <CheckCircle2 className="h-5 w-5 shrink-0 text-violet-700" /> : null}
                              </div>
                            </button>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        {[["now", "今から開始"], ["m5", "5分前"], ["m10", "10分前"], ["m30", "30分前"], ["manual", "仮で15分前"]].map(([value, label], index) => {
                          const active = startOffset === value;
                          return (
                            <button key={String(value)} onClick={() => setStartOffset(String(value))} className={`rounded-full px-4 py-2 text-sm transition ${active ? "bg-violet-600 text-white shadow-sm" : index % 2 === 0 ? "bg-pink-100 text-pink-700 hover:bg-pink-200" : "bg-sky-100 text-sky-700 hover:bg-sky-200"}`}>{label}</button>
                          );
                        })}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-3">
                      ...
                      <Dialog open={manualOpen} onOpenChange={setManualOpen}>
                        <DialogTrigger asChild>
                          <Button variant="outline" className="h-12 rounded-full border-violet-200 text-violet-700">手動ログ追加</Button>
                        </DialogTrigger>
                        <DialogContent className="max-w-xl rounded-[28px]">
                          <DialogHeader><DialogTitle>手動でログを追加</DialogTitle></DialogHeader>
                          <div className="grid gap-4">
                            <select className="rounded-2xl border border-slate-200 px-3 py-3" value={manualTaskId} onChange={(e) => setManualTaskId(e.target.value)}>
                            ...
                            <select className="rounded-2xl border border-slate-200 px-3 py-3" value={manualWorkTypeId} onChange={(e) => setManualWorkTypeId(e.target.value)}>
                              <option value="">作業項目を選ぶ</option>
                              {activeWorkTypes.map((workType) => <option key={workType.id} value={workType.id}>{workType.name}</option>)}
                            </select>
                            <Input type="datetime-local" value={manualStartAt} onChange={(e) => setManualStartAt(e.target.value)} className="rounded-2xl" />
                            <Input type="datetime-local" value={manualEndAt} onChange={(e) => setManualEndAt(e.target.value)} className="rounded-2xl" />
                            <Textarea value={manualNote} onChange={(e) => setManualNote(e.target.value)} placeholder="補足メモ（任意）" className="rounded-2xl" />
                            <Button className="rounded-full bg-violet-600 text-white hover:bg-violet-700" onClick={addManualLog}>手動ログを保存</Button>
                          </div>
                        </DialogContent>
                      </Dialog>
                    </div>
                ...
                <Card className="rounded-[30px] border border-violet-100 bg-gradient-to-b from-white to-violet-50/50 shadow-sm">
                  <CardHeader>
                    <SectionTitle icon={<CheckCircle2 className="h-5 w-5" />} title="この画面の狙い" sub="操作の軽さを最優先にした作り" />
                  </CardHeader>
                  <CardContent className="space-y-4 text-sm text-slate-600">
                    <div className="rounded-[24px] border border-pink-100 bg-pink-50/80 p-4">開始候補は進行中タスクだけに絞って、凍結や完了は混ぜないようにしています。</div>
                    <div className="rounded-[24px] border border-sky-100 bg-sky-50/80 p-4">開始後は自動で終了画面へ移動し、そのまま止める導線に乗れるようにしています。</div>
                    <div className="rounded-[24px] border border-violet-100 bg-white p-4">
                      <div className="font-medium text-slate-900">今の選択内容</div>
              ...
              <div className="grid gap-6 xl:grid-cols-[1.05fr_0.95fr]">
                <Card className="rounded-[30px] border border-sky-100 bg-white/85 shadow-sm">
                  ...
                  <CardContent className="space-y-6">
                    ...
                    <Button className="h-12 rounded-full bg-white px-8 text-violet-700 shadow-sm ring-1 ring-violet-200 hover:bg-violet-50" onClick={() => stopTimer(selectedStopTaskId, endNote)}><Square className="mr-2 h-4 w-4" />この作業を終える</Button>
                  </CardContent>
                </Card>

                <Card className="rounded-[30px] border border-violet-100 bg-white/85 shadow-sm">
                  <CardHeader>
                    <SectionTitle icon={<PauseCircle className="h-5 w-5" />} title="終了画面の狙い" sub="並列進行でも迷わず止められる" />
                  </CardHeader>
                  <CardContent className="space-y-4 text-sm text-slate-600">
                    <div className="rounded-[24px] border border-violet-100 bg-violet-50/70 p-4">終了時に作業項目を再入力させず、止める対象だけ選べば終われるようにしています。</div>
                    <div className="rounded-[24px] border border-pink-100 bg-pink-50/70 p-4">複数タスク進行中でも、進行中だけを並べることで終了対象が分かりやすくなります。</div>
                  </CardContent>
                </Card>
              </div>
            )}
            ...
            {tab === "tasks" && (
              <div className="grid gap-6">
                <Card className="rounded-[30px] border border-violet-100 bg-white/85 shadow-sm">
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="relative max-w-xl">
                      <Search className="absolute left-3 top-3 h-4 w-4 text-violet-400" />
                      <Input value={taskListSearch} onChange={(e) => setTaskListSearch(e.target.value)} placeholder="タスク名で検索" className="h-12 rounded-full border-violet-100 bg-white pl-10" />
                    </div>
                ...
                {(["active", "frozen", "completed"] as TaskStatus[]).map((status) => (
                  <Card key={status} className="rounded-[30px] border border-violet-100 bg-white/85 shadow-sm">
                    <CardHeader>
                      <SectionTitle icon={status === "active" ? <Activity className="h-5 w-5" /> : status === "frozen" ? <PauseCircle className="h-5 w-5" /> : <CheckCircle2 className="h-5 w-5" />} title={statusMeta[status].label} sub={status === "active" ? "今ふつうに触る制作物" : status === "frozen" ? "再開未定で寝かせている制作物" : "ひとまず完成した制作物"} />
                    </CardHeader>
                    <CardContent className="space-y-3">
                ...
                <Card className="rounded-[30px] border border-slate-200 bg-slate-50/70 shadow-sm">
                  <CardContent className="p-5 text-sm text-slate-600">アーカイブ {groupedTasks.archived.length} 件は日常導線から外す前提なので、ここでは軽い補足扱いにしています。</CardContent>
                </Card>
              </div>
            )}
              ...
              <div className="grid gap-6">
                <Card className="rounded-[30px] border border-violet-100 bg-white/85 shadow-sm">
                  ...
                  <CardContent className="grid gap-4 lg:grid-cols-3">
                    ...
                    <Card className="rounded-[24px] border border-sky-100 bg-sky-50/80 shadow-none"><CardContent className="p-4"><div className="text-sm text-slate-500">進行中</div><div className="mt-1 text-lg font-semibold">{runningTimers.length}件</div></CardContent></Card>
                  </CardContent>
                </Card>
                  ...
                  <Card className="rounded-[30px] border border-violet-100 bg-white/85 shadow-sm">
                    ...
                    <CardContent className="space-y-4">
                      {analysisByWorkType.length === 0 ? (
                        ...
                        <div key={row.workTypeId} className="rounded-[24px] border border-pink-100 bg-pink-50/50 p-4">
                          <div className="mb-2 flex items-center justify-between gap-3"><div className="font-medium">{row.name}</div><div className="text-sm font-medium">{formatMinutes(row.total)}</div></div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <ScrollArea className="h-[360px] rounded-[24px] border border-slate-100">
                      <div className="space-y-3 p-4">
                        {timeLogs.length === 0 ? (
                          <div className="rounded-[20px] border border-dashed border-slate-200 bg-slate-50 px-4 py-10 text-center text-sm text-slate-500">まだログがありません。</div>
                        ) : [...timeLogs].sort((a, b) => +new Date(b.endAt) - +new Date(a.endAt)).map((log) => (
                          <div key={log.id} className="rounded-[20px] border border-slate-200 bg-slate-50/80 p-3">
                            <div className="flex flex-wrap items-center justify-between gap-2">
                              <div>
                                <div className="font-medium">{tasksById[log.taskId]?.title}</div>
                                <div className="text-sm text-slate-500">{formatDateTime(log.startAt)} - {formatDateTime(log.endAt)} ・ {formatMinutes(log.durationMinutes)}</div>
                              </div>
                              <div className="flex gap-1">
                                <Badge className={`rounded-full border ${chipStyles[workTypesById[log.workTypeId]?.name ?? "その他"] ?? chipStyles["その他"]}`}>{workTypesById[log.workTypeId]?.name}</Badge>
                                <Button variant="ghost" className="rounded-full text-slate-700 hover:bg-white/80" onClick={() => openLogEdit(log)}><FileEdit className="h-4 w-4" /></Button>
                                <Button variant="ghost" className="rounded-full text-rose-700 hover:bg-white/80" onClick={() => deleteLog(log.id)}><Trash2 className="h-4 w-4" /></Button>
                              </div>
                            </div>
              ...
              <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
                <Card className="rounded-[30px] border border-amber-100 bg-white/85 shadow-sm">
                  <CardHeader>
                    ...
                    <div className="text-sm text-slate-500">削除はせず、無効化だけにして過去ログとの整合を保つ想定です。</div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {sortedWorkTypes.map((option, index) => (
                      <div key={option.id} className="flex items-center justify-between rounded-[22px] border border-amber-100 bg-amber-50/60 p-3">
                        <div className="flex items-center gap-3">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-white text-sm font-medium text-amber-700 shadow-sm">{index + 1}</div>
                          <div>
                            <div className="font-medium">{option.name}</div>
                            <div className="text-sm text-slate-500">{option.isActive ? "開始画面で選択可能" : "無効化中（過去ログには残る）"}</div>
                          </div>
                        </div>
                        <Button variant="outline" className="rounded-full border-amber-200 bg-white hover:bg-amber-50" onClick={() => toggleWorkTypeActive(option.id)}>{option.isActive ? "無効化" : "再有効化"}</Button>
                      </div>
                    ))}
                    <div className="rounded-[24px] border border-violet-100 bg-violet-50/50 p-4">
                ...
                <Card className="rounded-[30px] border border-violet-100 bg-violet-50/70 text-slate-900 shadow-sm">
                  <CardHeader>
                    <SectionTitle icon={<Settings2 className="h-5 w-5" />} title="バックアップと復元" sub="実運用時の安心感を上げるための導線" />
                  </CardHeader>
